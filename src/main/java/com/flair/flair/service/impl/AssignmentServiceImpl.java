...
public class AssignmentServiceImpl implements AssignmentService {
    ...
    public AssignmentServiceImpl(AssignmentRepository assignmentRepository, EmployeeRepository employeeRepository) {
    ...
    public void addEmployeesToService(Long assignmentId, Set<Long> employeesId) {
        AssignmentEntity assignmentEntity = this.assignmentRepository.findById(assignmentId).orElseThrow(() -> new
                AssignmentNotFoundException(assignmentId));
        Set<EmployeeEntity> employeeEntitySet = new HashSet<>(employeeRepository.findAllById(employeesId));
    ...
    public void addEmployeeToService(Long assignmentId, Long employeeId) {
        AssignmentEntity assignmentEntity = this.assignmentRepository.findById(assignmentId).orElseThrow(() -> new
                AssignmentNotFoundException(assignmentId));
        EmployeeEntity employeeEntity = employeeRepository.findById(employeeId).orElseThrow(() -> new EmployeeNotFoundException(employeeId));
