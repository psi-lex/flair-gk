...
public class NoteServiceImpl implements NoteService {
    ...
    public NoteServiceImpl(NoteRepository noteRepository, AssignmentRepository assignmentRepository, EmployeeRepository employeeRepository, NoteMapper noteMapper) {
    ...
    public void addNewNote(Long assignmentId, Long employeeId, NewNoteTo newNoteTo) {
        ...
        AssignmentEntity assignment = assignmentRepository.findById(assignmentId).orElseThrow(() -> new AssignmentNotFoundException(assignmentId));
        EmployeeEntity employee = employeeRepository.findById(employeeId).orElseThrow(() -> new EmployeeNotFoundException(employeeId));
    ...
    public void updateNote(Long assignmentId, Long employeeId, UpdateNoteTo updateNoteTo) {
        AssignmentEntity assignment = assignmentRepository.findById(assignmentId).orElseThrow(() -> new AssignmentNotFoundException(assignmentId));
        EmployeeEntity employee = employeeRepository.findById(employeeId).orElseThrow(() -> new EmployeeNotFoundException(employeeId));
        NoteEntity noteEntity = noteRepository.findById(updateNoteTo.getNoteId()).orElseThrow(() -> new NoteNotFoundException(updateNoteTo.getNoteId()));
    ...
    public void deleteNote(Long assignmentId, Long noteId) {
        AssignmentEntity assignment = assignmentRepository.findById(assignmentId).orElseThrow(() -> new AssignmentNotFoundException(assignmentId));
        NoteEntity noteEntity = noteRepository.findById(noteId).orElseThrow(() -> new NoteNotFoundException(noteId));
    ...
    public Set<NoteTo> displayAllNotes(Long assignmentId) {
        ...
        return assignment.getNoteEntitySet().stream()
                .map(noteMapper::mapNoteEntityToNoteTo).collect(Collectors.toSet());
    }
